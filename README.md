...
Bundle-processing persistence is provided by `PassiveDataKitPersistenceAdapter`. Existing deployments use `DatabasePassiveDataKitPersistenceAdapter` automatically, including bulk inserts for point and processing-trace batches and the configured ingest inspector. A deployment may configure another adapter class, factory, or dotted path with the `PDK_BUNDLE_PROCESSING_PERSISTENCE_ADAPTER` Django setting or `set_default_persistence_adapter()`. Each configured factory invocation must return a new adapter so concurrent workers do not share mutable state. Individual `BundleProcessingCore` instances and standalone bundle-processing writer functions may instead receive an explicit worker-scoped adapter instance. Custom adapters should extend the base class and explicitly select their point-inspection behavior; unimplemented operations fail rather than silently discarding writes.
